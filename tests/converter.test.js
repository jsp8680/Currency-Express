...
describe("Currency Converter", function () {
    ...
    it("should calculate the converted amount correctly", async function () {
        ...
        try {
            ...
            await fromSelect.findElement(By.css('option[value="AUD"]')).click();
            await toSelect.findElement(By.css('option[value="CAD"]')).click();
            const from = await fromSelect.getAttribute("value");
            const to = await toSelect.getAttribute("value");
            console.log("Currencies selected:", from, to);
            ...
            assert.equal(convertedAmount, "CAD 43.74", "Converted amount is incorrect");
        } catch (error) {
            console.error("An error occurred:", error.message);
            assert.fail(error.message);
        }
    ...
    it("should calculate the converted amount correctly", async function () {
        ...
        try {
            ...
            await fromSelect.findElement(By.css('option[value="EUR"]')).click();
            await toSelect.findElement(By.css('option[value="HKD"]')).click();
            const from = await fromSelect.getAttribute("value");
            const to = await toSelect.getAttribute("value");
            console.log("Currencies selected:", from, to);
            ...
            assert.equal(convertedAmount, "HKD 8510.90", "Converted amount is incorrect");
        } catch (error) {
            console.error("An error occurred:", error.message);
            assert.fail(error.message);
        }
